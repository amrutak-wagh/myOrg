({
    search : function(component, event, helper) {
        ...
        var excludedRecords = component.get('v.selection');
        var selectedSegments = component.get('v.selectedSegments');
        var parentId = '';

        for (var key in excludedRecords) {
            if (excludedRecords.hasOwnProperty(key)) {
                excludedRecordIds.push(excludedRecords[key].id);
            }
        }

        for (var key in selectedSegments) {
            if (selectedSegments.hasOwnProperty(key)) {
                parentId = selectedSegments[key].id;
            }
        }
        ...
        action.setParams({
            ...
            excludedRecordsIds: excludedRecordIds.toString()
        });
